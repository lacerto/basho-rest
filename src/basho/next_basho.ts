...
export default function getNextBasho(currentDate: moment.Moment): Basho {
    ...
    for (let key in bashoDB.basho_schedule) {
        ...
        let dateDelta: number = firstDay.diff(currentDate, 'days');
        if (dateDelta > 0) {
    ...
    let keyArray: Array<number> = [];
    Object.keys(schedules).forEach(element => keyArray.push(parseInt(element)));
    let key = Math.min(...keyArray);

    // Create an object with all the info about the next basho.
    return {
        description: schedules[key].description,
        venue: schedules[key].venue,
        firstDay: schedules[key].first_day,
        finalDay: schedules[key].final_day,
        previousFinalDay: prevBashoFinalDay,
        hasStarted: started
    };
}
